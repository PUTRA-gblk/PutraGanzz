...
try {
        ...
        mess = {
			...
			only: {
				group: 'Maaf Sepertinya Ini Khusus _*Group*_ !',
				ownerG: 'Maaf Sepertinya Ini Khusus _*Owner*_ !',
				ownerB: 'Maaf Sepertinya Ini Khusus _*Owner*_ !',
				admin: 'Maaf Sepertinya Ini Khusus _*Admin Group*_ !',
				Badmin: 'Maaf Jadikan Bot Sebagai _*Admin Group*_ !'
			}
		}
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© PutraGanzz_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `PutraGanzz`, "footerText": "*_© PutraGanzz_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© PutraGanzz_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
if(isButton == 'igm'){
reply('http://instagram.com/itsmeputt_21')
}
if(isButton == 'ytm'){
reply('http://youtube.com/c/Lord putra')
}
   ...
   const sotoy = [
		...
		//runtime berjalan
           runi = process.uptime() 
           kurr.setStatus(`Menunggu Mu Sejak ${kyun(runi)} `).catch((_)=>_);
...
│
└───「 \`\`\`PutraGanzz\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© PutraGanzz_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'📁 LISTMENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'👑 OWNER'},type:1},{buttonId:`script`,buttonText:{displayText:'📃 SC'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🔥 PUTTBOTZ 🔥'
       ownerJid = "6285854721440@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
...
//Sampai sini
case 'jadibot':
    if (isGroup) return reply('TIDAK BISA JADIBOT DIDALAM GRUP HARAP PC SAJA')
    if(mek.key.fromMe) return reply('Tidak bisa jadibot di dalam bot')
    jadibot(reply,kurr,from)
    break
    case 'stopjadibot':
    if (!isOwner && !isOwner) return reply(mess.only.owner)
...
ini_list.push({
"displayName": `Developer PUTTBOTZ`,
...
*│*⬡ *GOPAY    : ${gopay}*
*└───────────────────*
Donasi Bersyukur, Ga Donasi Gpp:)`
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				argzi = arg.split("|")
				if (!argzi) return reply(`Penggunaan ${prefix}spam teks|jumlah`)
				if (Number(argzi[1]) >= 99999) return reply('Kebanyakan!')
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`PutraGanzz`}, MessageType.location)
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY PutraGanzz`)
...
for (let i = 0; i < args[0]; i++) {
kurr.sendMessage(from, `PutraGanzz`, MessageType.extendedText,{
 quoted: {
key: {
...
kurr.sendMessage(from, `PutraGanzz`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer PutraGanzz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./thumb.jpg'),sourceUrl:"https://wa.me/p/6349482305092740/6281220670449"}}})
	}
    }
    }
    }
